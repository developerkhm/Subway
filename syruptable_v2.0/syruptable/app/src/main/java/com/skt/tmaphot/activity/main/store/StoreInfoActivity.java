...
import com.skt.tmaphot.net.model.store.StoreInfoBean;
...
public class StoreInfoActivity extends BaseActivity {
    ...
    private List<SocialListItem> socialReviewItemList;
    private Button socialReviewMore;
...
//
        ...
        APIClient.getInstance().getClient2(null).getStoreInfo(subpath, id)
                ...
                .subscribe(new Observer<StoreInfoBean>() {
                    ...
                    public void onNext(StoreInfoBean storeInfoBean) {
                        setStoreInfo(storeInfoBean);
                        socialReviewItemList = storeInfoBean.getSocialList();
                        setSocialReview(socialReviewItemList);
                    }
    ...
    public class ReviewImageRecyclerViewDataAdapter extends RecyclerView.Adapter<ReviewImageViewHolder> implements View.OnClickListener {
        ...
        public void onClick(View v) {
            Intent intent = new Intent(baceContext, ImageViewPagerActivity.class);
    ...
    private void initView() {
        ...
        info_review.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(baceContext, ReviewWriteActivity.class);
        ...
        socialReviewMore = (Button) findViewById(R.id.socialreview_more);
    }

    private void setStoreInfo(StoreInfoBean storeInfo) {
        ...
        if (isContent)
            ...
            storeContent.setVisibility(View.GONE);

        // Review Image
        storeInfoReviewRecyclerView = (RecyclerView) findViewById(R.id.storeinfo_review_recycler_view);
        if (storeInfo.getStore().getInsta() != null && storeInfo.getStore().getInsta().size() > 0) {
            ...
            storeInfoReviewRecyclerView.setVisibility(View.GONE);
        }
    }
    ...
    private void setSocialReview(List<SocialListItem> socialReview) {
        socialReviewRecyclerView = (RecyclerView) findViewById(R.id.socialreview_recycler_view);

        if (socialReview != null && socialReview.size() > 0) {
            LinearLayoutManager layoutManager = new LinearLayoutManager(getApplicationContext());
            layoutManager.setOrientation(LinearLayoutManager.VERTICAL);

            socialReviewRecyclerView.setLayoutManager(layoutManager);
            socialReviewRecyclerViewAdapter = new SocialReviewRecyclerViewAdapter(  socialReview.size() > 4 ? socialReview.subList(0, 5) : socialReview);
            ...
            if( socialReview.size() > 5){
                socialReviewMore.setVisibility(View.VISIBLE);
                socialReviewMore.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        Intent intent = new Intent(baceContext, SocialReviewTotalActivity.class);
                        intent.putParcelableArrayListExtra("social", (ArrayList<? extends Parcelable>) socialReviewItemList);
                        ActivityStart(intent, null);
                    }
                });
            }
            ...
            socialReviewRecyclerView.setVisibility(View.GONE);
        }
    }
}
