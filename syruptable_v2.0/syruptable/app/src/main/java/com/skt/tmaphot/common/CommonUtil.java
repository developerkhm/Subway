...
import com.skt.tmaphot.common.toast.StyleableToast;
...
//        return Base64.encodeToString(content.getBytes(), 0);
    ...
    public void custom_toast(Context context, String massage){
        new StyleableToast.Builder(context)
                .text(massage)
                .backgroundColor(getIntColor(context, R.color.colorGreen))
                .stroke(1, getIntColor(context, R.color.text_gray_8f))
                .show();
    }

    public int getIntColor(Context context, @ColorRes int id){
        if (Build.VERSION.SDK_INT >= 23) {
            // getResources().getColor(R.color.idname); // deprecated
            return ContextCompat.getColor(context, id);
        } else {
            return ResourcesCompat.getColor(context.getResources(), id, null); //without theme
        }
    }
}